...
func _write_type(type: DataType) -> NetCoreBuffer:
	_base.put_u8(type)
	return self
...
func set_data(bytes: PackedByteArray) -> NetCoreBuffer:
...
func seek(position: int) -> NetCoreBuffer:
	_base.seek(0)
	return self

func clear() -> NetCoreBuffer:
	seek(0)
	_base.clear()
	return self

func write_bool(value: bool) -> NetCoreBuffer:
	if value:
...
func write_int(value: int) -> NetCoreBuffer:
...
func write_bytes(bytes: PackedByteArray) -> NetCoreBuffer:
	if bytes.is_empty():
